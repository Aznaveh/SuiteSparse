...
#pragma omp atomic write
        ...
        {  // there is no CB, Nothing to be done
            if (zero_piv_rows > 0)
            {
                ...
                return paru_make_heap_empty_el
                    (f, pivotal_elements, hi, Work, Num);
            }
            else
            {
                Num->fcolCount[f] = 0;
        ...
        {     // NO rows for current contribution block
            if (zero_piv_rows > 0)
            {
                ...
                return 
                    paru_make_heap_empty_el(f, pivotal_elements, hi, Work, Num);
        ...
        PRLEVEL(-1, ("\n%%||||  Start Finalize %ld ||||\n", f));
        ParU_Ret res_prior;
        res_prior = paru_prior_assemble(f, start_fac,
                pivotal_elements, colHash, hi, Work, Num);
